#!/usr/bin/env node
// QVAC Business Slogan Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29362;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/slogan") {
      try {
        const { business } = await readBody(req);
        if (!business || !business.trim()) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "Please fill in the field first" }));
          return;
        }
        const result = await generate(modelId, business.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Business Slogan Generator GUI ready at http://localhost:${PORT}`);
  });
